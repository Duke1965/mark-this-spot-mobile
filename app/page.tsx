...
export default function PINITApp() {
  ...
  const [capturedMedia, setCapturedMedia] = useState<{
    ...
    type: "photo" | "video"
    location: string
    // optional extras when sharing PINIT pins
    title?: string
    description?: string
    personalThoughts?: string
    pinId?: string
      ...
      if (savedState) {
        ...
        if (
          ...
          [
            ...
            "results",
            "settings",
          ].includes(parsedState.currentScreen)
        ) {
  ...
  const getRealLocationName = async (lat: number, lng: number): Promise<string> => {
    ...
    try {
        ...
        try {
          ...
          if (geocodeResponse.ok) {
            ...
            if (geocodeData.results && geocodeData.results.length > 0) {
              ...
              const locality = addressComponents.find((comp: any) => comp.types.includes("locality") || comp.types.includes("sublocality"))
              if (locality) {
      ...
      if (isMobile) {
        ...
        else if (lat > -33.9 && lat < -33.8 && lng > 18.4 && lng < 18.5) return "Cape Town - CBD"
        else if (lat > -34.0 && lat < -33.9 && lng > 18.4 && lng < 18.5) return "Cape Town - Southern Suburbs"
        else if (lat > -33.9 && lat < -33.8 && lng > 18.4 && lng < 18.5) return "Cape Town - Northern Suburbs"
      ...
      if (isMobile) {
        if (lat > -33.8 && lat < -33.7 && lng > 18.9 && lng < 19.0) return "Riebeek West"
        else if (lat > -33.9 && lat < -33.8 && lng > 18.4 && lng < 18.5) return "Cape Town - CBD"
        else if (lat > -34.0 && lat < -33.9 && lng > 18.4 && lng < 18.5) return "Cape Town - Southern Suburbs"
        else if (lat > -33.9 && lat < -33.8 && lng > 18.4 && lng < 18.5) return "Cape Town - Northern Suburbs"
        else if (lat > -33.4 && lat < -33.3 && lng > 18.8 && lng < 18.9) return "Cape Town"
  ...
  const handleRecommendationAction = useCallback((action: string, data?: any) => {
  ...
  const handleProactiveSuggestion = useCallback((action: string, data?: any) => {
  ...
  // Fetch location photos (returns single best photo with filtering)
        ...
        if (closestPlace.photos && closestPlace.photos.length > 0) {
          const filteredPhotos = closestPlace.photos.filter((photo: any) => {
            if (photo.width && photo.height) {
              ...
              const isPortrait = aspectRatio < 0.5
              const isLandscape = aspectRatio > 2.5

              if (isSquareish || isTooSmall || isTooLarge || isPortrait || isLandscape) {
                console.log("📸 Filtering out photo:", {
                  dimensions: `${photo.width}x${photo.height}`,
                  aspectRatio: aspectRatio.toFixed(2),
                  reason: isSquareish ? "squareish (likely logo)" : isTooSmall ? "too small" : isTooLarge ? "too large" : isPortrait ? "too tall (likely sign)" : "too wide (likely banner)",
                })
                return false
              }
            }
  ...
  const handleRecommendationSubmit = (rating: number, review: string) => {
    ...
    if (recommendationData?.personalThoughts) {
      ...
      const newRecommendation: PinData = {
        ...
        personalThoughts: recommendationData.personalThoughts,
        tags: ["recommendation", "user-submitted", "pinit-pin", recommendationData?.platform || "social", ...(recommendationData?.aiTags || [])],
        isRecommended: true,
        rating: rating,
  ...
  // 🔁 Redirect editor → platform-select safely (no setState in render)
  useEffect(() => {
    if (currentScreen === "editor" && capturedMedia) {
      setCurrentScreen("platform-select")
    }
  }, [currentScreen, capturedMedia])
          ...
          if (capturedMedia.personalThoughts) {
            setTimeout(() => {
              ...
              setRecommendationData({
                mediaUrl: contentData.finalImageUrl || capturedMedia.url,
  ...
  if (currentScreen === "results" && currentResultPin) {
    return <PinResults pin={currentResultPin} onSave={handleSaveFromResults} onShare={handleShareFromResults} onBack={handleBackFromResults} />
  }
    ...
    return (
      <PinLibrary
        ...
        onPinUpdate={(pinId: string, updates: any) => {
          const updatedPins = pins.map((pin) => (pin.id === pinId ? { ...pin, ...updates } : pin))
  ...
  return (
    ...
    >
      ...
      {showRecommendationForm && recommendationData && (
        <RecommendationForm mediaUrl={recommendationData.mediaUrl} locationName={recommendationData.locationName} onRecommend={handleRecommendationSubmit} onSkip={handleRecommendationSkip} />
      )}
      ...
      {location && <EnhancedLocationService latitude={location.latitude} longitude={location.longitude} onLocationEnhanced={setLocationDetails} />}
      ...
      >
        ...
        <div
          style={{
            ...
            height: "360px",
            borderRadius: "50%",
            background: "radial-gradient(circle, rgba(255,255,255,0.8) 0%, rgba(255,255,255,0.5) 15%, rgba(255,255,255,0.2) 35%, transparent 50%)",
        ...
        <div
          style={{
            ...
            height: "400px",
            borderRadius: "50%",
            background: "radial-gradient(circle, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0.4) 15%, rgba(255,255,255,0.1) 35%, transparent 50%)",
          ...
          onMouseLeave={(e) => {
            if (!isQuickPinning) {
              ...
              e.currentTarget.style.boxShadow =
                motionData.isMoving && motionData.speed > 5 ? "0 8px 32px rgba(34, 197, 94, 0.4)" : "0 8px 32px rgba(0,0,0,0.3)"
            }
          }}
        >
          {(userLocation || location) && (
            ...
            >
              <img
                ...
                },${userLocation?.longitude || location?.longitude || 28.2293}&zoom=16&size=280x280&maptype=satellite&key=${
                ...
                onError={(e) => {
                  ...
                  ;(e.currentTarget as HTMLImageElement).src = `https://tile.openstreetmap.org/16/${Math.floor(
          ...
          >
            {isQuickPinning ? (
              ...
              <>
                <MapPin size={48} style={{ marginBottom: "0.5rem", color: "white", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.8))" }} />
                <span
                  style={{
                    color: "white",
                    textShadow: "0 2px 4px rgba(0,0,0,0.8)",
      ...
      {showNearbyPins && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: "bold", color: "white" }}>🌐 Real Places Nearby {isLoadingPlaces && "⏳"}</h3>
            <button
