...
const testLocationAccuracy = async () => {
  ...
  const testLocations = [
    {
      name: "Riebeek West Center",
      lat: -33.75,
      lng: 18.95,
      expected: "Riebeek West"
    },
    {
      name: "Riebeek West Residential",
      lat: -33.751,
      lng: 18.951,
      expected: "Riebeek West"
    },
    {
      name: "Near Allesverloren (2km away)",
      lat: -33.73,
      lng: 18.92,
      expected: "Should NOT show Allesverloren"
    },
    {
      name: "Riebeek West Edge",
      lat: -33.749,
      lng: 18.949,
      expected: "Riebeek West"
    }
  ...
  for (const test of testLocations) {
    ...
    try {
      const result = await getRealLocationName(test.lat, test.lng)
      console.log(`   ✅ Result: ${result}`)
      
      // Check if result matches expectation
      if (test.expected === "Should NOT show Allesverloren") {
        if (result.toLowerCase().includes("allesverloren")) {
          console.log(`   ❌ FAIL: Still showing Allesverloren instead of residential area`)
        } else {
          console.log(`   ✅ PASS: Correctly avoiding Allesverloren`)
        }
      } else if (result.toLowerCase().includes(test.expected.toLowerCase())) {
        console.log(`   ✅ PASS: Correctly identified ${test.expected}`)
      } else {
        console.log(`   ⚠️  PARTIAL: Got ${result}, expected ${test.expected}`)
      }
    } catch (error) {
  ...
  console.log("\n🌐 Testing Nearby Places Discovery...")
  for (const test of testLocations) {
    console.log(`\n🧪 Testing Nearby Places: ${test.name}`)
    
    try {
      // Simulate the findNearbyPins function
      const response = await fetch(`/api/places?lat=${test.lat}&lng=${test.lng}&radius=100`)
      ...
      if (response.ok) {
        ...
        if (places.length === 0) {
          console.log(`   ✅ Testing reverse geocoding fallback...`)
          // Test the reverse geocoding fallback
          const geocodeResponse = await fetch(
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${test.lat},${test.lng}&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`
          )
          ...
          if (geocodeResponse.ok) {
            ...
            if (geocodeData.results && geocodeData.results.length > 0) {
              ...
              console.log(`   ✅ Reverse geocoding fallback: ${areaName}`)
            }
          }
        } else {
  ...
  console.log("\n📸 Testing Photo Fetching...")
  for (const test of testLocations) {
    console.log(`\n🧪 Testing Photo Fetch: ${test.name}`)
    
    try {
      const photoResponse = await fetch(`/api/places?lat=${test.lat}&lng=${test.lng}&radius=50`)
      ...
      if (photoResponse.ok) {
        ...
        if (places.length > 0) {
          ...
          console.log(`   ✅ Business photo available: ${closestPlace.name}`)
        } else {
          console.log(`   ✅ No business photos - testing reverse geocoding fallback...`)
          
          // Test reverse geocoding fallback for photos
          const geocodeResponse = await fetch(
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${test.lat},${test.lng}&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`
          )
          ...
          if (geocodeResponse.ok) {
            ...
            if (geocodeData.results && geocodeData.results.length > 0) {
              ...
              console.log(`   ✅ Residential area name: ${areaName}`)
            }
          }
        }
      }
  ...
  console.log("✅ Residential area prioritization")
  console.log("✅ Allesverloren wine farm filtering")
  console.log("\n🚀 Ready for real-world testing in Riebeek West!")
}

// Add test function to window for easy testing
if (typeof window !== 'undefined') {
  (window as any).testLocationAccuracy = testLocationAccuracy
}
