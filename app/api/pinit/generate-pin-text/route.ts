...
import { areaOnlyFallbackDescription, sanitizePlaceDescription } from '@/lib/sanitizePlaceDescription'
...
- Be specific: say what stands out (food, view, history, vibe)—not "a nice place".
...
- Do NOT put street addresses, door numbers, or postal codes in the description. Use neighborhood, town, or region if helpful.
- If you lack facts, reference area (city/region) and category only—never paste a full address line.
...
provider: ${ctx.provider ?? ""}`;
  ...
  // If AI failed or returned invalid data, use deterministic fallback (no raw address lines)
  ...
  let desc = sanitizePlaceDescription(clampDesc(removeBannedPhrases(parsed?.description ?? "")));
  if (!desc) {
    desc = areaOnlyFallbackDescription({
      city: normalizeText(ctx.city),
      region: normalizeText(ctx.region),
    });
  }
  ...
  return {
    title: finalTitle,
    description: sanitizePlaceDescription(desc),
    confidence,
