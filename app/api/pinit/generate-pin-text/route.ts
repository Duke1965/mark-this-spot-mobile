...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Check if reverse geocode returned address/street - if so, we need POI search
    const isAddressOrStreet = geocodeData.place_type?.some((type: string) => 
    ...
    // Fetch POI if not cached OR if we have address/street and need to find best POI
    if (!poiData || (isAddressOrStreet && poiData.length > 0)) {
      ...
      try {
        ...
        if (!poiData) {
          poiData = [] // Empty array on failure only if we don't have cached data
        }
      }
    }
    
    // If reverse geocode returned address/street, use best POI for title/category/Wikimedia search
    let bestPOI: any = null
    let usePOIForMetadata = false
    
    if (isAddressOrStreet && poiData && poiData.length > 0) {
      // Find best POI (closest travel-related place within 200m)
      bestPOI = poiData.find((poi: any) => poi.distance_m && poi.distance_m <= 200) || poiData[0]
      usePOIForMetadata = true
      console.log(`📍 Address/street detected, using POI for metadata: ${bestPOI.name} (${bestPOI.distance_m}m away)`)
    }
    
    // Build response (keeping same shape for compatibility)
    // If we found a good POI for address/street, include it in geocode components
    ...
    if (usePOIForMetadata && bestPOI) {
      ...
      console.log(`✅ Using POI "${bestPOI.name}" for title/category/Wikimedia search`)
    }
    ...
    const response = {
      meta: {
        ...
        debug: {
          ...
          poi_candidates: poiData?.slice(0, 3).map((poi: any) => ({
            name: poi.name,
          ...
          wikimedia_search_term: usePOIForMetadata && bestPOI 
            ? bestPOI.name 
            : (geocodeData.feature_name || geocodeData.components?.name || '')
        }
      },
